...
public class ArticleVenduDAOJdbcImpl implements ArticleVenduDAO {
	...
	private static final String SELECT_CURRENT_WITH_FILTER_ALL_CAT = "SELECT no_article,no_categorie, nom_article, date_fin_encheres, pseudo, prix_initial,prix_vente FROM ARTICLES_VENDUS as a JOIN UTILISATEURS as u on a.no_utilisateur = u.no_utilisateur WHERE DATEDIFF(day, GETDATE(), date_fin_encheres) > 0 AND (nom_article LIKE ? OR description LIKE ?)";
	private static final String SELECT_CURRENT_WITH_FILTER_SINGLE_CAT = "SELECT no_article,no_categorie, nom_article, date_fin_encheres, pseudo, prix_initial,prix_vente FROM ARTICLES_VENDUS as a JOIN UTILISATEURS as u on a.no_utilisateur = u.no_utilisateur WHERE DATEDIFF(day, GETDATE(), date_fin_encheres) > 0 AND ((nom_article LIKE ? OR description LIKE ?) AND no_categorie = ?)";
