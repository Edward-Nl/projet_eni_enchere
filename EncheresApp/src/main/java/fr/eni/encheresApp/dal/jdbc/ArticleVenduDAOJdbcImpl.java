...
public class ArticleVenduDAOJdbcImpl implements ArticleVenduDAO {
	...
	private static final String SELECT_CORE = "SELECT no_article,no_categorie, nom_article, date_fin_encheres, pseudo, prix_initial,prix_vente FROM ARTICLES_VENDUS as a JOIN UTILISATEURS as u on a.no_utilisateur = u.no_utilisateur ";
	private static final String CURRENT_CONDITION = "DATEDIFF(day, GETDATE(), date_fin_encheres) > 0 AND DATEDIFF(day, GETDATE(), date_debut_encheres) < 0 ";
	private static final String END_CONDITION = "DATEDIFF(day, GETDATE(), date_fin_encheres) < 0 AND DATEDIFF(day, GETDATE(), date_debut_encheres) > 0  ";
	private static final String PSEUDO_CONDITION = "pseudo= ? ";
	private static final String WIN_CONDITION = "prix_vente = e.montant_enchere ";
	private static final String FILTER_CONDITION = "(nom_article LIKE ? OR description LIKE ?) ";
	private static final String CATEGORIE_CONDITION = "no_categorie = ? ";
	...
	public List<ArticleVendu> selectWithCondition(String pseudo, String filtre, int current, int categorie) {
		...
		boolean requeteCondition = false;
		requete.append(SELECT_CORE);

		switch (current) {
		case -1:
			requeteCondition = true;
			requete.append("WHERE ");
			requete.append(END_CONDITION);
			break;
		case 1:
			requeteCondition = true;
			requete.append("WHERE ");
			requete.append(CURRENT_CONDITION);
			break;
		case 0:
			break;
		default:
		...
		if (categorie != 0) {
			...
			requete.append(CATEGORIE_CONDITION);
		}
		...
		if (filtre != null && !filtre.trim().isEmpty()) {
			...
			requete.append(FILTER_CONDITION);
		}
		...
		if (pseudo != null && !pseudo.trim().isEmpty()) {
			...
			requete.append(PSEUDO_CONDITION);
		}
		...
		try (Connection cnx = ConnectionProvider.getConnection();
			...
			System.out.println("La " + pstmt.toString());
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
